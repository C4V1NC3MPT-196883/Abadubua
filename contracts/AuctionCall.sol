...
contract AuctionCall is Ownable {
    event NewOrderAlert(
        ...
        AuctionInstance.OrderDetail orderdetail
    );

    mapping(address => uint) Auction_Count;
    mapping(address => bool) Auction_inProgress;
    ...
    function SetAuctionLimit(uint _auction_limit) private onlyOwner {
        auction_limit = _auction_limit;
    }

    function RetractAuction(
        address auction_address
    ) public OutsourseImmutability(msg.sender, tx.origin) {
        require(
            Auction_inProgress[auction_address],
            "No auction with this address is in progress."
        );
        (bool hasbeen_retracted, bytes memory permission) = auction_address
            .call(
                abi.encodeWithSignature("isOwner_general(address)", msg.sender)
            );
        require(hasbeen_retracted, "The call of retract has failed.");
        require(
            abi.decode(permission, (bool)),
            "You are not the creator of this auction contract."
        );
        Auction_Count[msg.sender]--;
        delete Auction_inProgress[auction_address];
        (bool notin_progress, ) = auction_address.call(
            abi.encodeWithSignature("RetractMyAuction()")
        );
    ...
    function CreateNewAuction(
        ...
        public
        AuctionLimit(Auction_Count[msg.sender])
        OutsourseImmutability(msg.sender, tx.origin)
    {
        require(_minimalsplit <= _quantity, "Any split of quantity shouldn't be larger than the quantity in total.");
        AuctionInstance.OrderDetail memory orderdetail = AuctionInstance
            .OrderDetail(
                _orderinfo,
                _coal_category,
