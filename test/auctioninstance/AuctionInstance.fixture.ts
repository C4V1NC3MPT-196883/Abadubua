...
export async function getnewcontractfromdeploy(): Promise<Address[]> {
  ...
  const contractauctioncall = await contractFactory.connect(signers.alice).deploy(4); /**默认构造中的auction_limit=2  */
  ...
  //const contractComponent = await ethers.getContractFactory("AuctionInstance");
  //const contractauctioninstance = await contractComponent.connect(signers.)
